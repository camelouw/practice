...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count + 1; i++)
    {
        for (int j = i + 1; j < pair_count + 1; j++)
        {
            ...
            int iwin = pairs[i].win_amount;
            int jwin = pairs[j].win_amount;
...
*/
// locking all, removing when there is a cycle
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        {
            // check cycle
            if (locked[loser][pairs[j].winner] && winner == pairs[j].loser)
            {
...
void print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            if (locked[j][i])
            {
                candidate_winner = candidates[pairs[j].winner];
                break;
            }
        }
    }
...
*/
// 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